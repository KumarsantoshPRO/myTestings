...
export default class View extends Controller {
     ...
     */
    public onBulkExcelUploadPress(oEvent: any): void {
        const oInput = document.createElement("input");
        oInput.type = "file";
        oInput.accept = ".xlsx, .xls";

        oInput.onchange = async (e: any) => {
            ...
            oReader.onload = async (evt: any) => {
                try {
                    ...
                    const aRawRows = XLSX.utils.sheet_to_json(worksheet) as any[];
    ...
    private async _processBulkCustomers(aExcelData: any[]): Promise<void> {
        ...
        await this._executeCloudRequest("PUT", this.sCustomerBinId, this.sLocalCustomerKey, null, { customers: aExistingCustomers });
     ...
     */
    private async _processBulkAnalysis(aExcelData: any[]): Promise<void> {
        // 1. Pull down current historical analytics tracking lines
        const record = await this._executeCloudRequest("GET", this.sAnalyticsBinId, this.sLocalAnalyticsKey, { records: [] });
        const aExistingLogs: any[] = record.records || [];
            ...
            if (!bDuplicateExists && sDocId) {
                ...
                if (sRawDateStr) {
                    ...
                    if (aDateComponents.length === 3) {
                        const sFormattedIsoFallback = `${aDateComponents[2]}-${aDateComponents[1]}-${aDateComponents[0]}` +
                            (aDateTimeParts[1] ? `T${aDateTimeParts[1]}.000Z` : "T00:00:00.000Z");
        ...
        await this._executeCloudRequest("PUT", this.sAnalyticsBinId, this.sLocalAnalyticsKey, null, { records: aExistingLogs });
